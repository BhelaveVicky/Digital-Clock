    ...
    <div className="w-full mt-8 p-6 glass rounded-3xl animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {zones.map(zone => {
          ...
          return (
            <div key={zone.id} className="relative group p-4 bg-white/5 rounded-2xl border border-white/5 hover:border-white/20 transition-all">
              <button 
              ...
              <div className="text-white/60 text-xs mb-1 uppercase font-medium">{zone.city}</div>
              <div className="font-digital text-2xl text-white">
                {hours}:{minutes}
                <span className="text-sm opacity-60 ml-1">{seconds}</span>
                {!is24Hour && <span className="text-xs ml-1 opacity-60">{period}</span>}
              </div>
            </div>
          );
        })}
